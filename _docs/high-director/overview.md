...
For the Claude/Anthropic implementation, see [Build Your Own Sly Director]({{ '/docs/high-director/build-your-own-claude/' | relative_url }}). **Sly Director** is the Claude counterpart to High Director. It uses one persistent Sly Director Project with GitHub MCP and AWS MCP. Normal Project chat handles quick interactive work; Cowork is the primary mode for substantial long-running work so implementation plans can continue independently without repeated `continue` prompts. Skills and Plugins are optional upgrades, while Claude Code remains a specialist fallback.
